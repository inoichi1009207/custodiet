...
也就是说,「**最后一次快照之后新增、又在下一次快照前整段删除**的后缀」这个形态,
**外部锚与 `ledger-chain` 两件都不覆盖**(后者本就不防完整尾删,见 README)。
上一版这里只写「补上完整尾删」漏了限定词,再上一版把边界写成「当天」也不对
——正确的边界是**两次快照之间**,不是自然日:写入后若已发生过快照、之后再尾删,
当前长度小于快照长度,`verify` 会报 TRUNCATED。两处均已更正。
