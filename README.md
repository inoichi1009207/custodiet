...
- **CW——收尾时点再查一次并发写会话**:开工查一次只能证明那一刻没人在写;Stop 时再跑一次判活探针(`scripts/session-triage.mjs --json --fast`),非 PASS 即提示、不阻断。**本仓不带探针**,未接时本条每轮报一次 UNKNOWN 提示(不阻断);接上自己的即可(输出 JSON 里 `results[]` 含 `id:"①"` 一行,`status` 取 PASS/FAIL/UNKNOWN)。刻意**不做**「探针不在盘就静默」——那会让「探针被挪走」与「没有并发写」在闸的输出里长得一模一样。
