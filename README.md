...
- **台账自身有完整性链**:三份台账(报警分母/误报账/批目标史)加行级 hash 链,改写/插行/删中间行必红(`ledger-chain.mjs`;定性=完整性告警,担保边界见其头注——不防蓄意与完整尾删,我们不说做不到的话)。
  0.1.1 的**外部锚** `ledger-anchor.mjs` 只补到「**最后一次快照点之前**」那一段,
  且以「锚文件未被同身份进程改写 + `verify` 确实被运行」为前提;
  最后一次快照之后新增、又在下次快照前被整段删除的后缀,**两件都检不出**。
  外部锚**本仓未接唤醒面**(拷走后须自己接),`install-task` 目前只实现 Windows。
  详见 CHANGELOG 的 0.1.1 段与该文件头注——**别把这句读成「尾删问题已解决」**。
