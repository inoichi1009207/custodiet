...
- [unlazy](https://github.com/Leonxlnx/unlazy) (surveyed 2026-09) also uses a **Stop hook as a wall**, but treats the opposite disease: it fights **under-effort** (a Depth Tree splits the task N layers deep, every leaf inherits the full time budget; a gates file verifies with `CHECK:` commands + `EXPECT:` tokens), we fight **false claims** (did what it said it did actually happen; was the source actually fetched; was the verdict actually run). Its own docs say it does not cover false claims or provenance — exactly our home turf. Complementary rather than competing: **unlazy makes the agent work harder, custodiet makes it stop lying.** Two ideas worth borrowing: a progress guard that releases after N consecutive blocked stops with no progress instead of trapping, and an honest `ABANDON: <gate> <reason>` exit — our equivalents are the streak cap with auto-downgrade and "⏸ needs your confirmation + one of three legal reasons".
