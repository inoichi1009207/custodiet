...
- **CW — re-check concurrent writers at Stop time**: a start-of-session check only proves nobody was writing at that instant; at Stop the liveness probe (`scripts/session-triage.mjs --json --fast`) runs again and anything but PASS is surfaced as a notice, not a block. This repo does not ship the probe, so until you plug one in the rule surfaces one UNKNOWN notice per turn (never a block); wire up your own (JSON with a `results[]` entry `id:"①"` whose `status` is PASS/FAIL/UNKNOWN). Going silent when the probe is missing is deliberately **not** done: it would make "someone moved the probe" and "no concurrent writer" look identical in the gate's output.
...
- **External anchor** (0.1.1, `ledger-anchor.mjs`): periodically snapshots a **prefix fingerprint** of those three ledgers to a location outside the agent's write scope; `verify` compares. Anything rewritten, inserted, deleted or truncated **before the anchor point** goes red — the half a same-file forward chain structurally cannot cover. Commands: `node scripts/lib/ledger-anchor.mjs snapshot | verify | install-task`.
  **Where the guarantee stops (don't stop reading at the line above):**
  "External" here means **the same machine, the same user's home directory** (`~/.custodiet/`) — not another host, not another trust domain. Any process that can run commands as you can also rewrite the anchor; the anchor carries no signature and no chain of its own. It is a **soft boundary**: it raises the cost of forgery, it does not isolate.
  It covers only up to the **last snapshot point**, and only *given* that the anchor file itself was not rewritten by a same-identity process and that `verify` actually ran.
  Content appended after that snapshot and then deleted — while the file is still **at least as long as the snapshot** — is caught by **neither** mechanism. It does not have to be deleted "entirely".
  This repo does **not** wire a wake-up path for you, and `install-task` is Windows-only (`schtasks`) so far. See the 0.1.1 entry in CHANGELOG and that file's header — **do not read this as "truncation is solved."**
...
- [unlazy](https://github.com/Leonxlnx/unlazy) (surveyed 2026-09; every claim re-checked sentence-by-sentence by a second model against its README/SKILL) also uses a **Stop hook as a wall**, but treats the opposite disease: it fights **under-effort** (a Depth Tree splits the task by the requested depth and polishes every leaf over several passes; a gates file verifies with `CHECK:` commands + `EXPECT:` tokens), we fight **false claims** (did what it said it did actually happen; was the source actually fetched; was the verdict actually run). The ceiling its own docs admit is narrow: an `EXPECT` hit does not prove the gate title is honest, and transitive dependencies are not verified automatically — the whole "did it really happen" layer is exactly our home turf. Complementary rather than competing: **unlazy makes the agent work harder, custodiet makes it stop lying.** Two ideas worth borrowing: a progress guard that releases after 6 consecutive blocked stops with no progress instead of trapping, and a failure-style handoff `ABANDON: <id> <non-empty reason>` — our equivalents are the streak cap with auto-downgrade and "⏸ needs your confirmation + one of three legal reasons".
- Per our survey, the combination here — promise-vs-action comparison + mutation-tested acceptance + expiring debt ledger — was **not observed** in existing open source. (Survey date 2026-08-25, re-checked 2026-09-02; that's an observation, not a proof of absence.)
