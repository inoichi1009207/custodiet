...
//     ⚠️ **本段 2026-08-27 全文重写**(D66)。原文写着:防篡改日志的标准构造是
//     「哈希链 + 密钥演化 + 异机收锚」三件,本仓「三件里有一件半 ⇒ 原理上挡不住」。
//     **那个三要件是我拼出来的,不是文献结论;「一件半」的折算也无效。**
//     跨模型复核逐字核过两篇论文正文后判「已核实-不成立」,更正如下:
//
//     · Crosby & Wallach(USENIX Security '09)的核心构造是 **history tree**
//       (带签名承诺、成员证明、增量证明),经典哈希链只是被它比较的低效方案;
//       前向安全签名在原文里是**可选增强**——逐字:「Forward-secure digital signature
//       schemes [3] or stream authentication [21] **can be used** for signing commitments
//       in our scheme or any other logging scheme.」(该文 p.13, related work)
//       https://static.usenix.org/events/sec09/tech/full_papers/crosby.pdf
//     · VLDB 2004 "Tamper Detection in Audit Logs" 里,「哈希链 + MAC + 密钥演化 +
//       可信机器 T」那一段是在**介绍 Schneier–Kelsey**,不是该文自己的方案;
//       它自己走的是**站外公证 + 独立验证器**,并明写 Schneier–Kelsey 那套
//       「**these existing techniques are not applicable**」于其事务数据库场景。
//       其威胁模型对可信方的假设是逐字的:「The integrity of these services is assumed
//       to remain intact even in the event of a full DBMS compromise.」
//       https://www2.cs.arizona.edu/~rts/pubs/VLDB04.pdf
//
//     **正确的说法**:安全属性**不能按组件数相加**。本件的锚落在同一 OS 用户可控的
//     本机目录、且锚自身无来自独立主体的认证 ⇒ 具备同等写权限的攻击者可以把日志链、
//     摘要与锚一起重算。**真正承重的不是「补齐几件」,是四样**:
//       (a) **独立的信任边界**(见证状态不在攻击者控制域内——「另一台机器」若同凭据、
//           同管理域,仍可能是同一个失陷域,故「异机」本身不是判据);
//       (b) **时间边界**(篡改发生**之前**可信方已收到承诺;从初始化起就被控制则无解);
//       (c) **预先承诺**本身;(d) **审计真的发生**。
//     且即便四样齐备,给的通常也只是「对已承诺历史的**篡改检测**」——
//     不阻止删除、不保证攻击后的未来日志、不保证「该记而未记」的完整性、不保证可用性。
//     ⇒ 想变硬,第一步是**把见证状态移出本控制域**,不是补 forward-secure MAC。
//     **原文的错误归因值得留在这**:它把「信任边界问题」写成了「组件数量问题」,
//     于是把修复路线也指错了方向——这类错比「说得太满」更隐蔽,因为它方向是把自己说弱。
//     (出处:两篇论文正文均已由跨模型通道**真取页并逐字引出**,不再是摘要级。)
