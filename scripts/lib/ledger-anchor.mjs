...
//     ⚠️ **这个软是有学名的**(2026-08-26 批 111 工具面扫描第④层,键=「怎么知道一个进程
//     自己写的追加型日志有没有被同机同身份的东西改过」):防篡改日志的标准构造是
//     **哈希链 + 密钥演化(forward-secure MAC)**,外加**定期送到另一台可信机器**
//     ——见 Crosby & Wallach, USENIX Security '09(static.usenix.org/event/sec09/tech/
//     full_papers/crosby.pdf)与 VLDB 2004 "Tamper Detection in Audit Logs"
//     (www.vldb.org/conf/2004/RS13P1.PDF)。本仓的 ledger-chain **只做了哈希链那一半**,
//     没有密钥演化;本件补的「外部」也只到同机家目录,没有异机。
//     三件里我们有一件半 ⇒ 对**同权限攻击者**在原理上就挡不住,这不是实现没做好,
//     是构造本身缺了那两块。想要硬的,照文献补密钥演化 + 异机收锚,别指望调参。
//     (出处均为本轮真取页的搜索结果;两篇论文正文**未逐字核**,标为「已核-摘要级」。)
